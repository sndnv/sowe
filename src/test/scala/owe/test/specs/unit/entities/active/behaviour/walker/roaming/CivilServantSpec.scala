...
import owe.entities.active.behaviour.walker.roaming.CivilServant
import owe.map.grid.Point
import owe.test.specs.unit.AkkaUnitSpec
import owe.test.specs.unit.entities.active.behaviour.Fixtures
import owe.test.specs.unit.entities.active.behaviour.walker.WalkerBehaviour

class CivilServantSpec extends AkkaUnitSpec("CivilServantSpec") with WalkerBehaviour {

  private class TestCivilServant(
    ...
    override protected def createBehaviour(): BaseWalker = new CivilServant {}
  ...
  ("A CivilServant walker" should behave).like(
    roamingWalker(
      new TestCivilServant(properties, state, modifiers)
    )
  )
