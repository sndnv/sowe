...
import owe.entities.active.behaviour.walker.roaming.Resident
import owe.map.grid.Point
import owe.test.specs.unit.AkkaUnitSpec
import owe.test.specs.unit.entities.active.behaviour.Fixtures
import owe.test.specs.unit.entities.active.behaviour.walker.WalkerBehaviour

class ResidentSpec extends AkkaUnitSpec("ResidentSpec") with WalkerBehaviour {
  ...
  private class TestResident(
    ...
    override protected def createBehaviour(): BaseWalker = new Resident {}
  ...
  ("A Resident walker" should behave).like(
    roamingWalker(
      new TestResident(properties, state, modifiers)
    )
  )
